...
// Post detail modal
function NutritionistOtherPostDetailModal({
  postId,
  onClose,
}: {
  postId: string
...
export default function NutritionistOtherUserProfilePage() {
  ...
  return (
    <>
          ...
          {postIds.length === 0 && !isLoading ? (
            ...
            <div>
              <div className="grid grid-cols-3 gap-1">
                {postIds.map((id) => (
                  <NutritionistOtherGridCell key={id} postId={id} onSelect={setSelectedPostId} />
                ))}
              </div>
              {(isLoading || hasMore) && (
                <div ref={observerRef} className="py-8 flex justify-center">
                  {isLoading && <Loader2 className={`w-6 h-6 ${accent.text} animate-spin`} />}
                </div>
              )}
            </div>
          ) : (
      ...
      {selectedPostId && (
        <NutritionistOtherPostDetailModal
