  ...
  return (
    <>
      ...
      {selectedCertificate && (
        ...
        >
          <div 
            className="bg-[#1A1A1A] rounded-xl border border-white/10 max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-white/10">
              <h3 className="text-lg font-semibold text-white">{selectedCertificate.title}</h3>
              <button
                onClick={() => setSelectedCertificate(null)}
            ...
            <div className="flex-1 overflow-auto p-4 bg-[#0A0A0A]">
              <img
                src={`${API_URL.replace('/api', '')}${selectedCertificate.fileUrl}`}
