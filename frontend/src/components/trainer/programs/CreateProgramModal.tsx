...
interface CreateProgramModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (request: CreateProgramRequest) => Promise<void>;
}
...
export default function CreateProgramModal({ isOpen, onClose, onSubmit }: CreateProgramModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!coverImage) {
      alert(t.coverImageRequired || 'Cover image is required');
      return;
    }
    if (trainingVideos.length === 0) {
      alert(t.trainingVideosRequired || 'At least one training video is required');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            {!coverImage ? (
              ...
              >
                ...
                <span>{t.uploadCover || 'Upload Cover Image'}</span>
              </button>
            ) : (
          ...
          {/* Training Videos Upload */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {t.trainingVideos || 'Training Videos'}
            </label>
            <input
              ref={videosInputRef}
              type="file"
            ...
            >
              ...
              <span>{t.uploadVideos || 'Upload Training Videos'}</span>
            </button>
